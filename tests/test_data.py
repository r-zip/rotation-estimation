...
@pytest.mark.skipif(
    not DATA_PATH.exists(), reason="ShapeNetCore.v2 is not downloaded."
)
@pytest.mark.parametrize(
    "batch_size,points_per_sample", [(5, 100), (10, 1000)]
)
def test_point_cloud_dataloader(batch_size: int, points_per_sample: int):
    pc_dataloader = get_point_cloud_data_loader(
        DATA_PATH, batch_size=batch_size, points_per_sample=points_per_sample
    )
