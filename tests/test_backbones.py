...
from rotation_estimation.backbones import TNet, build_point_net
...
def test_point_net(batch_size: int, points: int, dim: int, output_dimension: int):
    x = torch.randn((batch_size, points, dim))
    pointnet = build_point_net(output_dimension=output_dimension)
    y = pointnet(x)
